    ...
    for key in masterclass_dict:
        ...
        if "jainam" in key.lower() and key==account_holder:
            ...
            for final_order_qty in order_qty:
                ...
                response = masterclass_dict[key].place_order(
...
def positions(req):
    ...
    ws_connection=response[3]
    ltp_cache=response[4]
    # print('Starting the Printing for positions')
    # for key,value in master_class_dict.items():
    #     print(value.get_positions()['data'])
    #     print('completed')
    ...
    for key in masterclass_dict:    
        ...
        for index, row in df.iterrows():
            ...
            pos.loc[len(pos)] = [instrument, expiry, strike, type_, qty, ltp, token, exchange]
...
def home(req):
    ...
    if req.method == "POST":
        ...
        for leg in legs:
            ...
            orders.append({
                ...
                "validity": "DAY",
                "user_order_id": "1",
                "price": float(price) if price!='' else ltp,
                "strike":strike,
                "expiry":expiry,
        ...
        for i in accounts_traded:
            for order in orders:
                order['quantity']=int(qty)*int(i[1])
                order_qty=[]
                match order['instrument']:
                    case 'BANKNIFTY':
                        while int(order['quantity'])>0:
                            if order['quantity']>banknifty_freeze_qty:
                                order_qty.append(banknifty_freeze_qty)
                                order['quantity']-=banknifty_freeze_qty
                            else:
                                lots=math.ceil(order['quantity']/banknifty_lot_size)
                                order_qty.append(lots*banknifty_lot_size)
                                order['quantity']-=(lots*banknifty_lot_size)
                    case 'NIFTY':
                        while int(order['quantity'])>0:
                            if order['quantity']>nifty_freeze_qty:
                                order_qty.append(nifty_freeze_qty)
                                order['quantity']-=nifty_freeze_qty
                            else:
                                lots=math.ceil(order['quantity']/nifty_lot_size)
                                order_qty.append(lots*nifty_lot_size)
                                order['quantity']-=(lots*nifty_lot_size)
                    case 'SENSEX':
                        while int(order['quantity'])>0:
                            if order['quantity']>sensex_freeze_qty:
                                order_qty.append(sensex_freeze_qty)
                                order['quantity']-=sensex_freeze_qty
                            else:
                                lots=math.ceil(order['quantity']/sensex_lot_size)
                                order_qty.append(lots*sensex_lot_size)
                                order['quantity']-=(lots*sensex_lot_size)
                if ("jainam" not in i[0].lower()):
                    ...
                    for final_order_qty in order_qty:
                        response = masterclass_dict[i[0]].place_order(
                            exchangeSegment=exchange_segment,
                            exchangeInstrumentID=exchange_token,
                            productType=product_type,
                            orderType=order_type,
    ...
    for key in masterclass_dict:
        ...
        # fut = temp_a[temp_a["symbol"].apply(lambda x: "BANKNIFTY" in x and "FUT" in x)]
        # xyz = sorted(list(fut["expiry"].unique()))
        # monthly_date_string_1 = dt.datetime.fromtimestamp(xyz[0]).strftime("%d-%m-%Y")
        # monthly_date_string_2 = dt.datetime.fromtimestamp(xyz[1]).strftime("%d-%m-%Y")
    # for key in masterclass_dict:
    #     contractz = masterclass_dict["Clean"].contracts["NFO"]
    #     break
